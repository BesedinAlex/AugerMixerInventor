...
namespace BesedinCoursework
{
    public partial class MainWindow : Form
    {
        ...
        private void Plane(int OccurrenceOne, int PartPlaneOne, int OccurrenceTwo, int PartPlaneTwo, string Offset = "0", bool MateOrFlush = false) // Совмещение базовых плоскостей
        {
            ...
            if (MateOrFlush == true) oAssCompDef.Constraints.AddMateConstraint(oAsmPlane1, oAsmPlane2, Offset);
            else oAssCompDef.Constraints.AddFlushConstraint(oAsmPlane1, oAsmPlane2, Offset);
        }
        ...
        private void Surface(int OccurrenceOne, int PartFaceOne, int OccurrenceTwo, int PartFaceTwo, double Offset = 0, bool MateOrFlush = true) // Совмещение поверхностей
        {
            ...
            if (MateOrFlush == true) oAssCompDef.Constraints.AddMateConstraint(oFace1, oFace2, Offset);
            else oAssCompDef.Constraints.AddFlushConstraint(oFace1, oFace2, Offset);
        }
        ...
        private void Build_Click(object sender, EventArgs e) // Выполнение сборки
        {
            ...
            for (int I = 0; I < PartIndex.Length; I++) oAssDoc.ComponentDefinition.Occurrences.Add(oFileName[PartIndex[I]], ThisApplication.TransientGeometry.CreateMatrix());
            // Присоединение крышки
            Surface(1, 1, 2, 19);
            Surface(1, 2, 2, 20);
            try { Surface(1, 9, 2, 114); }
            catch { Surface(1, 9, 2, 113); }
            // Присоединение шнека
            Axis(1, 3, 3, 3);
            Plane(1, 3, 3, 3);
